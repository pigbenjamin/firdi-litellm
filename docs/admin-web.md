...
| 頁面 | 路徑 | 用途 |
|---|---|---|
| 總覽 | `GET /api/v1/admin/web` | 身分、可管理部門總覽 |
| 模型清單 | `GET /api/v1/admin/web/models` | 名稱／狀態／類型／上游／本期用量與額度／點數費率／測試結果／已授權部門。**含 YAML 定義的地端模型**（標「既有」） |
| 模型詳情 | `GET /api/v1/admin/web/models/detail?model_name=…` | 單一模型的全部欄位、各部門實際花費，以及測試呼叫／發布／停用／重新啟用／永久刪除 |
...
| 個人點數上限 | `GET/POST /api/v1/admin/web/access/users/points?user_id=…` | 只存值（跨所有模型合計），不扣點、不擋——外部系統讀 `GET /api/v1/users` 自己判斷 |
...
| **已發布** | 是（前提是有授權） | 有 | 只有描述性欄位：顯示名稱、額度、點數費率、備註 |
...
- **額度是整個模型的總量，不是每個部門各自的。** `model_spend` 雖然按部門分列
  （見下方模型詳情頁的「各部門實際花費」），額度判斷讀取時會跨部門加總。
...
模型詳情頁有一塊「各部門實際花費」，依真實用量算出來，不是手填的標籤。舊制的
`cost_center` 欄位（一個模型手動指定一個部門）已經停用——一個模型可能被多個部門
共用同一把 key，單一標籤標不出真正的分帳。

`config/custom_logger.py` 每次成功呼叫時，除了記到 `model_spend` 的整體累計，也會
一併記下呼叫者的部門（`dept_id`）；沒有部門脈絡的呼叫（地端模型健康檢查、curl 測試
等）記到「（未分類）」桶，不會被丟掉或報錯。額度上限的判斷邏輯完全不變——讀取時
跨部門加總，仍然是整個模型的總量。

## 個人點數上限：只存值

個人授權編輯頁（`/access/users/edit?user_id=…`）多一格「個人點數上限」，連到
`/access/users/points?user_id=…`——可以設一個**跨所有模型合計**的點數上限與週期
（每月／累計）。**這裡不扣點、不擋任何呼叫**，`config/custom_auth.py` 完全不看
這兩個欄位，跟模型的點數費率是同一種取捨：只是設定介面，實際扣點與擋人的判斷
由外部系統處理，讀既有的 `GET /api/v1/users` 就能拿到（`points_limit`／
`points_period` 兩個欄位已經在回應裡，不需要新開端點）。

**留空是「還沒設上限」，不是 0。** 0 點在外部系統眼裡等於「完全不能用」，跟「還沒
設定」是兩回事。
...
（顯示名稱、模型類型、額度上限、點數費率、備註）。填過一次的組合可以
...
**決策 E 時期的 `dept:<provider>` 部門 key 機制已於 2026-09 全面拔除**——不管是
admin-web 或 `ADMIN_API_KEY` 的 curl 路徑，上架時都一律要求明確的 `api_key`，
不再有「留空就退回部門 key」這條路；沒帶 key 直接 422，不會默默留下一個打不通的
模型。
...
儲存／刪除範本、設定部門模型授權、設定個人模型授權、設定個人點數上限、推送授權到
OpenWebUI、從 OpenWebUI 拉回授權。
...
- **地端模型的額度形同虛設**：LiteLLM 算不出它們的成本，`response_cost` 是 `null`，
  用量累計不到。
- **額度是模型層級的總量**，不能依部門切分（雖然 `model_spend` 有記各部門的分量）。
- **點數費率只是記錄**：這個平台不扣點、不檢查點數上限、也不會因為點數用完而擋下
  呼叫。扣點與部門／人員的總點數上限由外部系統處理（見上方「點數費率：只存不算」
  與「個人點數上限：只存值」）。
- **一律模型自帶 key**（2026-09 起）：舊制的部門專屬 key（`dept:<provider>`）機制
  已全面拔除，沒有例外。
